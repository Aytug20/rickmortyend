...
class CharacterDetailFragment : Fragment() {
    ...
    fun observeLiveData(){
        viewModel.characterLiveData.observe(viewLifecycleOwner, Observer{ character->
            character?.let{
                charactersId.text=it.id.toString()
                charactersName.text=it.name
                charactersStatus.text=it.status
                charactersSpecies.text=it.species
                charactersGender.text=it.gender
